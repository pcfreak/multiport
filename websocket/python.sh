...
wget -q -O /usr/local/bin/stunnelws https://Tarap-Kuhing/multiport/main/Websocket/stunnelws.py
chmod +x /usr/local/bin/stunnelws

# Installing Service
cat > /etc/systemd/system/stunnelws.service << END
[Unit]
Description=SSH WEBSOCKET TLS ROUTING INDONESIA BY TARAP KUHING
...
# Getting Proxy Template
wget -q -O /usr/local/bin/nontls https://raw.githubusercontent.com/Tarap-Kuhing/multiport/main/websocket/nontls.py
chmod +x /usr/local/bin/nontls

# Installing Service
cat > /etc/systemd/system/nontls.service << END
[Unit]
Description=SSH WEBSOCKET NON TLS ROUTING INDONESIA BY TARAP KUHING
Documentation=https://t.me/zerossl
After=network.target nss-lookup.target
...
# Getting Proxy Template
wget -q -O /usr/local/bin/otls https://raw.githubusercontent.com/Tarap-Kuhing/multiport/main/websocket/otls.py
chmod +x /usr/local/bin/otls
# Installing Service
cat > /etc/systemd/system/otls.service << END
[Unit]
Description=OVPN WEBSOCKET TLS ROUTING INDONESIA BY TARAP KUHING
Documentation=https://t.me/zerossl
After=network.target nss-lookup.target
...
# Getting Proxy Template
wget -q -O /usr/local/bin/openvpnws https://raw.githubusercontent.com/Tarap-Kuhing/multiport/main/websocket/openvpnws.py
chmod +x /usr/local/bin/openvpnws

# Installing Service
cat > /etc/systemd/system/openvpnws.service << END
[Unit]
Description=OVPN WEBSOCKET ROTING PENGKOL BY TARAP KUHING
Documentation=https://t.me/zerossl
After=network.target nss-lookup.target
